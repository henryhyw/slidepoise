...
The Editorial photographs and depicted marks come from its retained generated targets. These are historical full-slide designs, including the earlier inconsistent folios. The earlier folio calibration retained the original artwork. A subsequent illustration revision regenerated the three small collages on page 3 individually with genuine RGBA transparency, using the original crops as references. Their width increased from roughly 400 pixels to roughly 1,300 pixels. The Agent inspected light and dark composites and the actual PowerPoint render. Canvas placement and all twelve native objects on that page remain unchanged. Exact requests, source crops, returned files and source bindings are retained under that page’s `work/illustrations/` directory. Its authored artwork regions remain raster while the surrounding presentation text stays native. Future generated designs follow the current body-only and inherited-frame policy. The Consulting chart retains its numeric data and native labels. Its feedback routes are editable freeforms. Automatic reattachment when moving related objects is not claimed.
