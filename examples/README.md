...
Each example includes a PowerPoint, previews, the selected designs and the inputs needed to reconstruct them. `showcase.json` identifies the current public assets using paths relative to its example directory. Earlier experiments are available in Git history.
...
The recorded previews use LibreOffice on macOS with Arial and Georgia, plus Courier New in the Editorial deck. Other fonts or renderers may change text wrapping.

These are reproducible construction inputs. Creating a new presentation starts with a new brief and Agent-led design. Start a new run with current Profiles instead of copying a sample’s resolved configuration.
