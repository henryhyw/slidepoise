...
The same publication role appeared under different local entity names. `run/work/deck-design.json` records their shared treatment and aliases. Every reconstruction handoff binds that decision by hash, while the semantic styles implement it. A role declaration records the decision and does not apply typography on its own.
...
The original generated targets remain as historical full-slide designs. This native calibration used no new image call, so the old folio placements remain visible in target comparisons. New presentations follow the current content-only generation and inherited-frame policy.
...
`run/bundle/deck-scenes.json` is the portable scene entry point. `run/work/rebuild-verification.json` records the current checks. The source ZIP includes that bundle and the complete authoring evidence. The project page offers one PowerPoint download. PDF and source files remain available in the repository.
