# A controlled path to shared AI capability
...
The chart values and numeric labels are linked to its workbook. Its vertical columns preserve the generated design's orientation and category order. Separate formula text and scenario table cells are editable and do not recalculate automatically. Named native fonts differ from generated lettering. Some table entries use additional lines to preserve their common font size.

The sample also exercises the complete visual-asset path. The Agent selected line and fill candidates before generation, reviewed the variants in the accepted composition, and bound each icon role to its canonical asset ID. Reconstruction restores the chosen SVG files instead of copying the generated stand-ins.
