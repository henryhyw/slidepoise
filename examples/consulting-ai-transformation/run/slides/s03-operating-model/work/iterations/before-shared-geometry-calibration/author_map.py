...
def text(identifier, value, box, size, group, *, bold=False, family="Arial", role="body", color="#000000", align="left", leading=1.0, search=None):
    item = {"id": identifier, "display_label": value.replace("\n", " "), "kind": "text", "role": identifier,
            ...
            "style_hint": {"font_family": family, "font_weight": "bold" if bold else "regular",
                           ...
                           "vertical_alignment": "top", "observed_foreground_color": color,
...
text("headline", "Keep expert ownership at every release", [46, 7, 1825, 88], 64, "claim-title", family="Georgia", role="slide_title", search=[46, 25, 1122, 59])
text("subtitle", "A controlled path from permissioned evidence to approved client work.", [47, 97, 1825, 51], 32, "subtitle", role="subtitle", search=[47, 108, 1160, 34])
...
shape("decision-band", [48,793,1824,127], "#FFF5ED")
shape("decision-accent", [48,793,8,127], "#FD5108")
text("decision-label", "RELEASE RULE", [112,837,304,47], 28, "decision-band", bold=True, color="#FD5108", role="call_to_action", search=[112,839,246,38])
text("decision-message", "No client use without expert and engagement-lead sign-off.", [426,837,1400,47], 28, "decision-band", bold=True, role="call_to_action", search=[421,842,1250,37])
...
groups = [
    ...
    {"id":"release-decision","display_label":"Expert and engagement-lead sign-off","semantic_class":"release rule","group_reason":"The shared decision band makes client release conditional on both sign-offs.","children":["decision-band","decision-accent","decision-label","decision-message"],"bbox_hint":[48,793,1824,127]},
]
...
handoff = {"selected_assets":[],"sources":["Host-authored illustrative Northstar Advisory planning model"],
    ...
    "The decision band's orange bar uses the shared8-pixel specification. Its shallow overall placement follows the accepted source."
]}
